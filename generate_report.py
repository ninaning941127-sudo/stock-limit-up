...
def build_report(stats_dict, bench_ret, bench_start, bench_end):
    ...
    add_paragraph(
        doc,
        "白話解讀：僅鎖死組平均每天賺 3.07%，p 值小到幾乎是 0，代表這不太可能是巧合；"
        "僅未鎖死組平均每天虧 3.88%，同樣是極端顯著的虧損；兩者混在一起的整體，平均每天"
        "賺 0.94%，也顯著大於 0。三個結果都指向同一個結論：是顯著的——鎖死賺、未鎖死虧"
        "這個規律在半年、上百個交易日裡反覆出現，不是少數幾天的好運撐出來的。"
    )
    add_paragraph(
        doc,
